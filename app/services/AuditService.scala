...
import models.{SubmissionAuditDetail, SubmissionDataForAuditModel}
...
trait AuditService {
  ...
  def sendTAVCSubmitAdvancedAssuranceEvent(submittedDataForAudit: SubmissionDataForAuditModel, tavcReferenceNumber: String,
                                           responseReceived: HttpResponse, acknowledgementRef: String)
                                          (implicit hc: HeaderCarrier, rh: RequestHeader): Unit = {

    val failureReason: String = if (checkResponseSuccess(responseReceived.status)) AuditConstants.notApplicable
    else
      getResponseReason(responseReceived).fold(AuditConstants.noValueProvided)(_.toString)

    val np = AuditConstants.noValueProvided

    val detailData = SubmissionAuditDetail(
    ...
    auditConnector.sendEvent(ExtendedDataEvent.apply(appName, AuditConstants.submitAdvancesAssuranceAuditType,
